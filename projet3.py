...
from Gradient_Minimum import * #importe toutes les fonctions de ce programme /!\ les 2 programmes doivent être enregistré dans la même dossier.
...
T[NX-1] = T0*Rapport_temp #Température à l'extérieur = là où il y a le feu, il fait 4 fois plus chaud
...
def cost(): #évalue le cout sur NX valeurs de CNTRL
    cost = []
    for i in range(NX): #On cherche la quantité optimale de CNTRL à mettre
        CNTRL = i
        cost += [J(i)]
    return cost
